...
const PET_ICONS: Record<number, string> = {
  1: 'panda.svg',
  2: 'lemur.svg',
  3: 'gorilla.svg',
  4: 'alligator.svg',
...
function getIconForPet(petId: number): string {
  return ICON_BASE + (PET_ICONS[petId] ?? 'pet-placeholder.svg');
}

function getImageForPet(petId: number): string {
...
async function loadCameras(): Promise<void> {
  try {
    ...
    const camData = (await camRes.json()) as { data: Camera[] };
    const petData = (await petRes.json()) as { data: Pet[] };
...
function openZoosSidebar(): void {
  sidebar?.classList.add('is-expanded');
}

function closeZoosSidebar(): void {
